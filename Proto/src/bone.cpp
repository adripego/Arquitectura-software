...
void Bone::Render() {
    ...
    Renderer::Instance().translateMatrix(currentX, currentY, 0.f);
    Renderer::Instance().RotateMatrix(currentRotation, 0, 0, -1);
  
    if (this->GetImage() != NULL)
    {
        this->image->SetHandle(this->handleX * this->GetImage()->GetWidth(), this->handleY * this->GetImage()->GetHeight());
        Renderer::Instance().DrawImage(this->GetImage(), 0, 0, 0, currentScaleX * this->GetImage()->GetWidth(), currentScaleY * this->GetImage()->GetHeight(), 0);
        Renderer::Instance().translateMatrix(pivotX * this->GetImage()->GetWidth(), pivotY * this->GetImage()->GetHeight(), 0);
    }
...
double Bone::Interpolate(int32 id, int32 prevId, int32 nextId, double prevVal, double nextVal) const {
	// TAREA: Implementar la especificacion del enunciado
    double interp;
    interp = prevVal + (nextVal - prevVal) * (static_cast<double>(id)-static_cast<double>(prevId)) / (static_cast<double>(nextId)-static_cast<double>(prevId));
    return interp;
}
